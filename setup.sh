...
print_next_steps() {
    ...
    if [ ${#INSTALLED_ITEMS[@]} -gt 0 ]; then
        echo -e "${YELLOW}→${NC} Reload your shell environment:"
        echo -e "  ${BLUE}source ~/.bashrc${NC}"
        echo ""
    fi
    
    echo -e "${YELLOW}→${NC} Verify installations:"
    echo -e "  ${BLUE}go version${NC}          # Should show >= 1.21"
    echo -e "  ${BLUE}python3 --version${NC}   # Should show >= 3.11"
    echo -e "  ${BLUE}uv --version${NC}        # Should show uv version"
    echo ""
    
    echo -e "${YELLOW}→${NC} Clone and build the Claude Code Hooks Monitor:"
    echo -e "  ${BLUE}git clone https://github.com/INS-JVidal/claude-hooks-monitor.git${NC}"
    echo -e "  ${BLUE}cd claude-hooks-monitor${NC}"
    echo -e "  ${BLUE}make build${NC}"
    echo ""

    echo -e "${YELLOW}→${NC} Start the monitor and test:"
    echo -e "  ${BLUE}make run${NC}            # Start the monitor server"
    echo -e "  ${BLUE}# In another terminal:${NC}"
    echo -e "  ${BLUE}make test${NC}           # Run the test suite"
    echo ""

    echo -e "${GREEN}All system tools are ready!${NC}"
    echo -e "${GREEN}Clone the repo and run 'make build' to get started.${NC}"
}
